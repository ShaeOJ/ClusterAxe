...
// Periodically logs per-task runtime accounting (delta over the interval plus
// lifetime totals) to the console. Useful for spotting a task that is hogging
// the CPU or a stack that is running low across the cluster firmware.
...
// Maintains a smoothed system CPU-utilization figure in
// GLOBAL_STATE->SYSTEM_MODULE.cpu_usage (exposed as cpuUsage in the API).
