...
static void check_master_device(void)
{
    ...
    if (reason != THROTTLE_NONE) {
        ...
        // Safe now, but stay throttled (user can manually restore)
        // Just clear the reason
...
// Note: Unthrottling is not automatic - user should manually restore settings
// via the web UI or API after resolving the thermal/power issue
