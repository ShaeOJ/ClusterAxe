@echo off
echo ============================================
echo  ClusterAxe Build Script - GT 801 Slave
echo  GammaTurbo, 2x BM1370, 12V, cluster slave
echo  Build dir: build_gt_slave  Config: sdkconfig.gt-slave
echo ============================================
echo.
...
:: Build into a dedicated dir with the GT slave config as SDKCONFIG. A
...
%IDF_PYTHON% %IDF_PATH%\tools\idf.py -B build_gt_slave -D SDKCONFIG_DEFAULTS="%CD%\sdkconfig.defaults;%CD%\sdkconfig.defaults.gt-slave" -D SDKCONFIG=%CD%\build_gt_slave\sdkconfig -D NPM_EXECUTABLE=%NPM_EXE% build
...
copy /Y build_gt_slave\zombie-os-slave.bin build_gt_slave\clusteraxe-gt801-slave.bin >nul
...
echo ============================================
echo.
echo Firmware files located in: build_gt_slave\
echo  - clusteraxe-gt801-slave.bin (copy of app, variant-named^)
echo  - zombie-os-slave.bin (main application^)
echo  - bootloader\bootloader.bin
echo  - partition_table\partition-table.bin
echo  - www.bin
echo.
echo Flash from build_gt_slave\ (see the esptool line idf.py printed above^).
echo.
