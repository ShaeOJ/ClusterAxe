#!/usr/bin/env python3
# Convert the built Angular web UI (dist/axe-os) into a C source file of byte
# arrays that gets compiled directly into the firmware. This replaces the old
# separate SPIFFS "www" partition image (www.bin): the default UI now ships
# inside the app binary, so a normal flash needs no www.bin at all.
#
# The SPIFFS "www" partition is retained for the optional "custom www" override
# (uploading a replacement UI via /api/system/OTAWWW) -- see http_server.c.
import os
import sys
